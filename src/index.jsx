...
function MUIEditor({
  ...
  const toolbarVisibleConfig = editorFactories.getConfigItem(
    "toolbar",
    "visible"
  );
  const [isToolbarVisible, setIsToolbarVisible] = React.useState(
    toolbarVisibleConfig
  );
  const [
    isResizeImageDialogVisible,
    setIsResizeImageDialogVisible,
  ...
  const bottom =
    editorFactories.getToolbarPosition() === "bottom" ? toolbar : null;
  ...
  const editorWrapperProps = {
    ...
    }} ${editorFactories.getConfigItem("editor", "className")}`,
    onClick: () => editorRef.current.focus(),
  };

  const editorWrapperElement = editorFactories.getConfigItem(
    "editor",
    "wrapperElement"
  );
