...
function ImageControl({ configuration, defaultConfiguration }) {
  ...
  const uploadCallback =
    configuration.uploadCallback || defaultConfiguration.uploadCallback;

  const imageEntityToResize = editor.resizeImageEntityKey
    ? editor.editorState
        .getCurrentContent()
        .getEntity(editor.resizeImageEntityKey)
    : null;
  ...
  const handleResizeImage = (width, height) => {
    ...
    const newEditorState = EditorState.push(
      editor.editorState,
      contentState.mergeEntityData(editor.resizeImageEntityKey, {
        width,
        height,
      }),
      "apply-entity"
  ...
  return (
    <React.Fragment>
      ...
      <ResizeImageDialog
        ...
        originalWidth={
          imageEntityToResize ? imageEntityToResize.getData().width : 0
        }
        originalHeight={
          imageEntityToResize ? imageEntityToResize.getData().height : 0
        }
