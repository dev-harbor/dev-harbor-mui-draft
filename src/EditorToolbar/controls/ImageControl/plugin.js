...
const EditorImage = ({ src, width, height, contentState, block }) => {
  ...
  const imageAlign = (ev, align) => {
    ...
    editor.onChange(
      EditorState.push(editor.editorState, newContentState, "change-block-data")
    );
    editorFocus();
  };
  ...
  const removeImage = (ev) => {
    ...
    const selectionToStart = SelectionState.createEmpty(
      firstBlock.getKey()
    ).merge({
    ...
    editor.onChange(
      EditorState.push(editor.editorState, newContentState, "remove-range")
    );
    editorFocus();
  };
